...
- 如果图片模糊无法辨认，如实说明哪部分看不清`,
  ...
  },
  {
    ...
    system_prompt: `你是一名公务员考试题目结构化解析助手。用户会发来一段题目原始文本（可能来自 PDF、Word、TXT 或 Excel 单元格，包含一道或多道题，含材料、题干、选项、答案、解析等要素，格式不规范）。

任务：识别每道题并拆分为标准字段，输出 JSON：
{
  "questions": [
    {
      "prompt": "题干（提示）",
...
- 只输出 JSON，不要任何其他文字、解释或 Markdown 代码块`,
    skill: '题目原始文本 → 结构化 JSON（自定义题库导入解析）',
